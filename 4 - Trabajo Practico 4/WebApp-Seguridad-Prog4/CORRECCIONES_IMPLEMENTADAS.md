...
router.post('/api/process-image', validateUpload, async (req, res) => {
  try {
    ...
    const result = await executeCommand('convert', [
      filePath,
...
router.post('/api/upload', authenticateUser, upload.single('file'), async (req, res) => {
  try {
...
const { queryLimiter, validateAgainstSQLInjection } = require('../middleware/blindSQLProtection');
